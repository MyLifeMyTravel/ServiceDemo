...
### 10.1.2 Service-小试牛刀
新建一个项目 **ServiceDemo**，在 `service` 包下创建 `FirstService.kt` 类(mac:Command + N)，Source Language 选择 `Kotlin`。

![WechatIMG234](http://odsdowehg.bkt.clouddn.com/WechatIMG234.jpeg)
...
现在 `FirstService.kt` 中只有一个构造函数和一个 `onBind()` 方法。通过查看 `Service` 的源码，我们可以发现 `onBind()` 方法是 `Service` 中唯一的一个抽象方法，该方法用于 Bound Service。关于 Bound Service 会在后面小结会讲到。这里我们复写 `onStartCommand`、`onCreate()`、`onDestroy()` 方法，并在这些方法中添加 Log 来验证服务是否开启。
...
创建 `BoundServiceActivity.kt`，代码如下:
...
接下来是 `ServiceLifeCycleActivity.kt`，代码如下：
...
本章代码已全部上传至Github:[ServiceDemo](https://github.com/MyLifeMyTravel/ServiceDemo/tree/kotlin)。文中的每个例子都是一次commit，直接 checkout 就可以看到完整可运行的 Demo 了。Java 版本请见[ServiceDemo Kotlin](https://github.com/MyLifeMyTravel/ServiceDemo)
